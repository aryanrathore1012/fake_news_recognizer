...
class fake_news_predictor:
    ...
    def __init__(self): # file path change in this function
        ...
        # self.the_big_one_path = "the_big_one.csv"
        # self.tile_image_path = "tile_image.ico"
        # self.icon_path = "icon.ico"

        # change these variables to run the program 
        self.the_big_one_path = "F://aryans_code_notes//machine_learning//fake_news_predictor//the_big_one.csv"
        self.tile_image_path = "F://aryans_code_notes//machine_learning//fake_news_predictor//tile_image.jpg"
        ...
        Y_ = train_data["label"].astype('int')

        # now the program is more accurate
        tf_idf_vectorizer = TfidfVectorizer()
        X_vector= tf_idf_vectorizer.fit_transform(X_)  

        X_train, X_test, Y_train, Y_test = train_test_split(X_vector, Y_, test_size=0.2, random_state=2)
        ...
        self.random_forest_model = RandomForestClassifier(n_jobs=2, random_state=1)
        self.random_forest_model.fit(X_vector,Y_)
        ...
        # Label(top_frame,text=f"Enter the title and author of the news\nthe model's train score: {self.train_score:.3f}, test score: {self.test_score:.3f}",font=("Cascadia Mono SemiBold", 20, "bold italic"),fg='black',bg="#D5CEA3",anchor=CENTER).pack(side=TOP,pady=(10,25),fill=X)
        ...
        def check_title_author():
            ...
            if author_var.get() == "":
                ...
                transformed_sentance = tf_idf_vectorizer.fit_transform([stemmed_sentance])
